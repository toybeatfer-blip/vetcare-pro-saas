...
export const MasterTenantsManagement: React.FC = () => {
  ...
  const handleSendCredentialsWhatsApp = () => {
    ...
    let msg = `Hola Dr(a). *${adminUserForm.name || selectedTenantForUsers.directorName}*, le saludamos del soporte técnico del Software Veterinario.\n\n`;
  ...
  const filteredTenants = useMemo(() => {
    ...
    return tenants.filter((tenant) => {
      const matchesSearch =
        ...
        tenant.directorName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        tenant.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
        tenant.phone.includes(searchTerm) ||
        (tenant.city && tenant.city.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (tenant.adminCredentials && tenant.adminCredentials.username.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (tenant.encargadoCredentials && tenant.encargadoCredentials.username.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const handleSendWhatsApp = (tenant: TenantClinic, customKey?: string) => {
    const cleanPhone = tenant.phone.replace(/[^0-9]/g, '');
    const expDate = new Date(tenant.expirationDate).toLocaleDateString('es-MX', {
      day: '2-digit',
      month: 'long',
      year: 'numeric',
    });

    let msg = `Hola Dr(a). ${tenant.directorName}, le saludamos del equipo técnico del Software Veterinario.\n\n`;
    if (customKey) {
  ...
  const handleSendPaymentWhatsApp = (req: PaymentRenewalRequest) => {
    ...
    let msg = `Hola Dr(a). ${req.directorName}, le saludamos de la Dirección del Software Veterinario.\n\n`;
    if (isApproved) {
      msg += `✅ Le confirmamos que su pago de *$${req.amount} MXN* (Referencia: *${req.referenceFolio}*) ha sido acreditado exitosamente.\n\nSu licencia para *${req.clinicName}* ha sido reactivada por *${req.plan === 'mensual' ? '+30 días (+1 Mes)' : '+365 días (+1 Año)'}*.\n\nPuede ingresar normalmente al sistema.\n¡Muchas gracias por su preferencia!`;
    } else {
      msg += `Hemos recibido su referencia de renovación para *${req.clinicName}* con folio *${req.referenceFolio}* por *$${req.amount} MXN* (${req.paymentMethod.toUpperCase()}).\n\nSu comprobante está en proceso de validación y la reactivación se dará en un plazo máximo de 24 horas hábiles.`;
    }
    window.open(`https://wa.me/${cleanPhone}?text=${encodeURIComponent(msg)}`, '_blank');
  };
                  ...
                  <div className="space-y-2 flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="px-2 py-0.5 rounded-lg bg-indigo-50 text-indigo-700 border border-indigo-200 text-xs font-bold uppercase">
                        {req.paymentMethod === 'card' ? '💳 Tarjeta Bancaria' : req.paymentMethod === 'spei' ? '⚡ SPEI' : '🏪 OXXO'}
                      </span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs pt-1">
                      ...
                      <div>
                        ...
                        <span className="font-black text-emerald-700 text-sm">
                          ${req.amount.toLocaleString('es-MX')} MXN ({req.plan === 'mensual' ? '+30 Días' : '+365 Días'})
                        </span>
                      </div>
                      <div>
