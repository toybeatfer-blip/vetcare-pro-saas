...
export const VeterinaryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const registerNewClinic = (data: RegisterClinicData): boolean => {
    try {
      ...
      // 2. Clear medical records and patients for a clean start
      setPets([]);
      setMedicalRecords([]);
      setVaccines([]);
      setAppointments([]);
    ...
    try {
      ...
      try {
        ...
        if (paymentsRes?.success && Array.isArray(paymentsRes.requests)) {
          if (paymentsRes.requests.length > paymentRequests.length) {
            newPaymentsCount = paymentsRes.requests.length - paymentRequests.length;
          }
          setPaymentRequests(paymentsRes.requests);
          localStorage.setItem(LOCAL_STORAGE_KEYS.PAYMENT_REQUESTS, JSON.stringify(paymentsRes.requests));
        }
      } catch {
        // Local fallback
      }
  ...
  const isLicenseLocked = useMemo(() => {
    ...
    return false;
  }, [currentUser?.role, systemLicense.isLocked, systemLicense.status, daysRemaining]);
